...
public class SimpleBuilderOLD {
	...
	private final static Logger LOG = Logger.getLogger(SimpleBuilderOLD.class);
	
	protected GraphicsElement svgRoot;
	
	protected SVGPrimitivesOLD derivedPrimitives;
	protected SVGPrimitivesOLD rawPrimitives;
	...
	private Path2ShapeConverterOLD path2ShapeConverter = new Path2ShapeConverterOLD();
	
	protected long timeout;
	protected long startTime;

	public SimpleBuilderOLD(GraphicsElement svgRoot) {
		setSvgRoot(svgRoot);
		this.timeout = Long.MAX_VALUE;
	}

	public SimpleBuilderOLD(GraphicsElement svgRoot, long timeout) {
		setSvgRoot(svgRoot);
	...
	private void fillRawPrimitivesLists() {
		if (rawPrimitives == null) {
			rawPrimitives = new SVGPrimitivesOLD();
			...
			rawPrimitives.addImages(SVGImage.extractSelfAndDescendantImages(svgRoot));
		}
	}
	...
	public void createDerivedPrimitives() {
		if (derivedPrimitives == null) {
			...
			derivedPrimitives = new SVGPrimitivesOLD(rawPrimitives);
			...
			ListIterator<SVGPath> pathsIt = derivedPrimitives.getPathList().listIterator();
			while (pathsIt.hasNext()) {
				SVGPath path = pathsIt.next();
				...
				/*if (shape instanceof SVGLine) {
					derivedPrimitives.add((SVGLine) shape);
				} else if (shape instanceof SVGRect) {
					derivedPrimitives.add((SVGRect) shape);
	...
	private void convertLinelikePolygonsToLines() {
		ListIterator<SVGPolygon> polygonsIt = derivedPrimitives.getPolygonList().listIterator();
		while (polygonsIt.hasNext()) {
			SVGPolygon polygon = polygonsIt.next();
			SVGLine line = path2ShapeConverter.createNarrowLine(polygon);
			if (line != null) {
				derivedPrimitives.add(line);
				polygonsIt.remove();
			}
		}
		ListIterator<SVGRect> rectsIt = derivedPrimitives.getRectList().listIterator();
	...
	private void removeZeroLengthLinesAndEmptyPaths() {
		...
		Iterator<SVGPath> j = derivedPrimitives.getPathList().iterator();
		paths: while (j.hasNext()) {
			SVGPath line = j.next();
	...
	private void removeWhiteFilledPolygons() {
		...
		while (polygonsIt.hasNext()) {
			SVGPolygon polygon = polygonsIt.next();
			if ((polygon.getStyle() != null && polygon.getStyle().contains("fill:#ffffff")) || (polygon.getAttribute("fill") != null && polygon.getAttribute("fill").getValue().equals("#ffffff"))) {
				polygonsIt.remove();
			}
		}
	}
	...
	protected void sewTogetherCutLines() {
		List<SVGLine> allLines = new ArrayList<SVGLine>(derivedPrimitives.getLineList());
		for (SVGPolygon polygon : derivedPrimitives.getPolygonList()) {
			allLines.addAll(polygon.createLineList(true));
		}
		
		Iterator<SVGLine> lines1 = derivedPrimitives.getLineList().iterator();
		while (lines1.hasNext()) {
			SVGLine line1 = lines1.next();
			line2: for (SVGLine line2: derivedPrimitives.getLineList()) {
				if (line1 == line2) {
					continue;
				}
	...
	protected void sewTogetherCutPolygons() {
		...
		outer: while (polygonsIt.hasNext()) {
			...
			for (SVGPolygon polygon2 : derivedPrimitives.getPolygonList()) {
					...
					if (linesDoNotIntersect) {
						...
						for (SVGLine line3: allLines) {
							if (lineInWay(line3, joiningLines.get(0)) || lineInWay(line3, joiningLines.get(0))) {
								Real2Array newPoints = getConvexHull(polygon1, polygon2);
	...
	private Real2Array getConvexHull(SVGPolygon polygon1, SVGPolygon polygon2) {
		Real2Array p1Points = polygon1.getReal2Array();
		Real2Array p2Points = polygon2.getReal2Array();
		Point2DOLD[] points = new Point2DOLD[p1Points.size() + p2Points.size()];
		int i = 0;
		for (Real2 p : p1Points) {
			points[i] = new Point2DOLD(p.getX(), p.getY());
			i++;
		}
		for (Real2 p : p2Points) {
			points[i] = new Point2DOLD(p.getX(), p.getY());
			i++;
		}
		GrahamScanOLD hull = new GrahamScanOLD(points);
		Real2Array newPoints = new Real2Array();
		for (Point2DOLD p : hull.hull()) {
			newPoints.add(new Real2(p.x(), p.y()));
		}
		return newPoints;
	}
	...
	protected void removeNearDuplicateAndObscuredPrimitives() {
		...
		while (i.hasNext()) {
			SVGPolygon p = i.next();
			Iterator<SVGLine> j = derivedPrimitives.getLineList().iterator();
			while (j.hasNext()) {
	...
	private void removeNearDuplicatePolygons() {
		Iterator<SVGPolygon> j = derivedPrimitives.getPolygonList().iterator();
		while (j.hasNext()) {
			SVGPolygon polygon1 = j.next();
			polygon2: for (SVGPolygon polygon2 : derivedPrimitives.getPolygonList()) {
				checkTime("Took too long to look for near-duplicate and obscured primitives");
	...
	private void convertPolylinesToPolygonsOrSetsOfLines() {
		...
		while (i.hasNext()) {
			...
			if (polygon1 != null) {
				...
				//addId(i, shape, path);
				derivedPrimitives.addShapeToSubclassedLists(polygon1);
				//LOG.trace("Lines " + (derivedPrimitives.getLineList() == null ? 0 : derivedPrimitives.getLineList().size()));
				i.remove();
			} else {
				SVGPolygon result = convertFilledPolylineToPolygon(polyline);
				if (result == null) {
					derivedPrimitives.addLines(polyline.createLineList());
				} else {
	...
	private void convertUnfilledPolygonsToSetsOfLines() {
		...
		while (i.hasNext()) {
			...
			if ((polygon.getStyle() != null && polygon.getStyle().contains("fill:none")) || (polygon.getAttribute("fill") != null && polygon.getAttribute("fill").getValue().equals("none"))) {
				derivedPrimitives.addLines(polygon.createLineList(true));
				i.remove();
			}
		}
	}
	...
	private void convertPathsToShapes() {
		...
		derivedPrimitives.getPathList().addAll(newPaths);
	}
	...
	public static void abstractPolygon(SVGPolygon polygon, double maxAngleForCorner) {
		abstractPoly(polygon, true, maxAngleForCorner);
	}
	
	public static void abstractPolyline(SVGPolyline polyline, double maxAngleForCorner) {
		abstractPoly(polyline, false, maxAngleForCorner);
	}
	...
	protected static void abstractPoly(SVGPoly poly, boolean closed, double maxAngleForCorner) {
		...
		while (currentPoints.size() > 3) {
			...
			if (areaWithSmallestChange / area < 0.65) {
				break;
			}
	...
	public SVGPrimitivesOLD getDerivedPrimitives() {
		return derivedPrimitives;
	}

	public SVGPrimitivesOLD getRawPrimitives() {
