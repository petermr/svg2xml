...
//	String id;
	...
	SVGXTreeEdge getParentEdge() {
		...
		if (tempVarComplexLine != null) {
			List<Joint> joints = tempVarComplexLine.getJoints(treeAnalyzer.treeSideOrientation.getOtherOrientation()); 
			if (joints.size() == 0) {
	...
	public void addText(String title, String value) {
		
		text = new SVGText(this.getXY().plus(new Real2(textSize, textSize/2.)), value);
