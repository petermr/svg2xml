...
import org.xmlcml.svg2xml.text.FontNormalizerOLD;
...
public class TextAnalyzerOLD extends ChunkAnalyzer {

	private final static Logger LOG = Logger.getLogger(TextAnalyzerOLD.class);
    ...
    private TextAnalyzerOLD rot0TextAnalyzer = null;
    private TextAnalyzerOLD rotPi2TextAnalyzer = null;
    private TextAnalyzerOLD rotPiTextAnalyzer = null;
    private TextAnalyzerOLD rot3Pi2TextAnalyzer = null;
    private TextAnalyzerOLD rotIrregularTextAnalyzer = null;
    private FontNormalizerOLD fontNormalizer;
	...
	public TextAnalyzerOLD() {
		this((PageAnalyzer)null);
	}
	public TextAnalyzerOLD(PageAnalyzer pageAnalyzer) {
		super(pageAnalyzer);
	}
	
	private TextAnalyzerOLD(PageAnalyzer pageAnalyzer, TextOrientation textOrientation) {
		this(pageAnalyzer);
		this.textOrientation = textOrientation;
	}
	
	public TextAnalyzerOLD(List<SVGText> textList, PageAnalyzer pageAnalyzer) {
		super(pageAnalyzer);
		setTextList(textList);
	}

	public TextAnalyzerOLD(List<SVGText> textList) {
		super((PageAnalyzer) null);
		setTextList(textList);
	}

	public TextAnalyzerOLD(GraphicsElement svgElement, PageAnalyzer pageAnalyzer) {
	...
	private void getRawCharacterList(List<SVGElement> textElements) {
		rawCharacterList = new TextLineOLD(this);
	...
	public List<TextLineOLD> getTextLines() {
	...
	private TextStructurerOLD ensureTextContainerWithSortedLines() {
		if (textStructurer == null) {
			textStructurer = TextStructurerOLD.createTextStructurerWithSortedLines(textCharacters, this);
		} else {
	...
	private void ensureFontNormalizer() {
		if (fontNormalizer == null) {
			fontNormalizer = FontNormalizerOLD.getDefaultNormalizer();
		}
	}
	...
	public List<AbstractContainerOLD> createContainers() {
		TextStructurerOLD textStructurer1 = this.getTextStructurer();
		textStructurer1.getScriptedLineListForCommonestFont();
		List<TextStructurerOLD> splitList = textStructurer1.splitOnFontBoldChange(-1);
		List<TextStructurerOLD> textStructurerList = splitList;
		...
		for (TextStructurerOLD textStructurer : textStructurerList) {
			ScriptContainerOLD scriptContainer = ScriptContainerOLD.createScriptContainer(textStructurer, pageAnalyzer);
	...
	private void splitBoldHeaderOnFontSize(List<TextStructurerOLD> textStructurerList) {
		TextStructurerOLD textStructurer0 = textStructurerList.get(0);
		if (textStructurer0.getScriptedLineListForCommonestFont().size() > 1) {
			textStructurer0.getScriptedLineListForCommonestFont();
			List<TextStructurerOLD> splitList = textStructurer0.splitOnFontSizeChange(999);
			List<TextStructurerOLD> fontSplitList = splitList;
	...
	public void createRotatedTextAnalyzers() {
		if (rot0TextAnalyzer == null ||
			...
			rot0TextAnalyzer = new TextAnalyzerOLD(pageAnalyzer, TextOrientation.ROT_0);
			rotPi2TextAnalyzer = new TextAnalyzerOLD(pageAnalyzer, TextOrientation.ROT_PI2);
			rotPiTextAnalyzer = new TextAnalyzerOLD(pageAnalyzer, TextOrientation.ROT_PI);
			rot3Pi2TextAnalyzer = new TextAnalyzerOLD(pageAnalyzer, TextOrientation.ROT_3PI2);
			rotIrregularTextAnalyzer = new TextAnalyzerOLD(pageAnalyzer, TextOrientation.IRREGULAR);
