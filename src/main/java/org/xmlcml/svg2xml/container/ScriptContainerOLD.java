...
import org.xmlcml.svg2xml.text.StyleSpanOLD;
...
public class ScriptContainerOLD extends AbstractContainerOLD implements Iterable<ScriptLineOLD> {
	...
	public final static Logger LOG = Logger.getLogger(ScriptContainerOLD.class);
	...
	public ScriptContainerOLD() {
		super((PageAnalyzer) null);
	}
	public ScriptContainerOLD(PageAnalyzer pageAnalyzer) {
		super(pageAnalyzer);
	}

	public ScriptContainerOLD(List<ScriptLineOLD> scriptedLineList) {
		this();
		this.add(scriptedLineList);
	}
	
	public static ScriptContainerOLD createScriptContainer(TextStructurerOLD textStructurer, PageAnalyzer pageAnalyzer) {
		List<TextLineOLD> textLineList = textStructurer.getTextLineList();
		for (TextLineOLD textLine : textLineList) {
			LOG.trace("TLSC "+textLine);
		}
		ScriptContainerOLD scriptContainer = new ScriptContainerOLD(pageAnalyzer);
		List<ScriptLineOLD> scriptedLineList = textStructurer.getScriptedLineListForCommonestFont();
		for (ScriptLineOLD scriptLine : scriptedLineList) {
	...
	private void setTextStructurer(TextStructurerOLD textStructurer) {
	...
	private void possiblyAddParaBeforeLine(RealRange xRange, ScriptLineOLD lastLine,
			ScriptLineOLD scriptLine) {
	...
	private void possiblyAddParaAfterLine(RealRange xRange, ScriptLineOLD scriptLine) {
		if (scriptLine.couldEndParagraph(xRange)) {
			htmlElement.appendChild(new HtmlP());
		}
	}

	private void addIndividualSpans(StyleSpansOLD styleSpans) {
		for (int j = 0; j < styleSpans.size(); j++) {
			StyleSpanOLD styleSpan = styleSpans.get(j);
	...
	private void ensureScriptList() {
		if (scriptLineList == null) {
			this.scriptLineList = new ArrayList<ScriptLineOLD>();
		}
	}
	...
	public void add(List<ScriptLineOLD> scriptLineList) {
		ensureScriptList();
	...
	public Double getLargestFontSize() {
		Double fontSize = null;
		for (ScriptLineOLD script : scriptLineList) {
...
//			}
	...
	public Iterator<ScriptLineOLD> iterator() {
	...
	Multiset<Double> createLeftIndentSet(int decimalPlaces) {
		if (leftIndentSet == null) {
			...
			for (ScriptLineOLD scriptLine : this) {
	...
	public static ScriptContainerOLD createScriptContainer(File file) {
...
//		svgPage.appendChild(svgg);
		...
		ScriptContainerOLD sc = ScriptContainerOLD.createScriptContainer(textStructurer, pageAnalyzer);
		return sc;
	}
