...
public class WhitespaceChunkerXTest {
	...
	public void testPageSplitter0() throws Exception {
		...
		SVGSVG svgPage = (SVGSVG) SVGElement.readAndCreateSVG(SVG2XMLFixtures.PAGE0_SVG);
		...
		SVG2XMLFixtures.drawChunkBoxes(finalChunkList);
	...
	public void testHarter3() throws Exception {
		SVGSVG svgPage = (SVGSVG) SVGElement.readAndCreateSVG(SVG2XMLFixtures.HARTER3_SVG);
		WhitespaceChunkerAnalyzerX pageChunkSplitterAnalyzer = new WhitespaceChunkerAnalyzerX();
		List<Chunk> finalChunkList = pageChunkSplitterAnalyzer.splitByWhitespace(svgPage);
		Assert.assertNotNull(finalChunkList);
	...
	public void testHarter3small() throws Exception {
		...
		SVGSVG svgPage = (SVGSVG) SVGElement.readAndCreateSVG(SVG2XMLFixtures.HARTER3SMALL_SVG);
		WhitespaceChunkerAnalyzerX pageChunkSplitterAnalyzer = new WhitespaceChunkerAnalyzerX();
		List<Chunk> finalChunkList = pageChunkSplitterAnalyzer.splitByWhitespace(svgPage);
		Assert.assertNotNull(finalChunkList);
		//Assert.assertEquals("chunks", 29, finalChunkList.size());
	...
	public void testPolicies() {
		splitFileAndOutputNoTest(SVG2XMLFixtures.POLICIES_SVG, 7, new File(WHITESPACE+"/policies.svg"));
	}
	
	@Test
	public void testPage6() {
		splitFileAndOutputNoTest(SVG2XMLFixtures.AJC6_SVG, 14, new File(WHITESPACE+"/ajc_page6_split.svg"));
	}
